...
class StrandsDocr(BaseDocr):
    ...
    def fetch_content(self, url: str) -> Document:
        """Fetch and parse a Strands documentation page.

        Strands docs are in markdown format, so we just fetch the raw content.

        Args:
            url: Documentation page URL (markdown file)

        Returns:
            Document with full markdown content
        ...
        return Document(url=url, content=content, metadata={"title": title, "source": "strands", "format": "markdown"})
