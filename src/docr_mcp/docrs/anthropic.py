...
class AnthropicDocr(BaseDocr):
    ...
    def _parse_llms_txt(self, content: str) -> List[IndexEntry]:
        ...
        for line in lines:
            ...
            # Detect section headers (## Section Name or ### Subsection)
            ...
            if section_match:
                ...
                if level == 2:
                    ...
                    if len(current_section) > 0:
                        ...
                        section_stack.append((0, section_name))
                continue
            ...
            # Some Anthropic entries have: - [title](url) - description
            link_match = re.match(r"[*-]\s+\[(.+?)\]\((.+?)\)(?:\s+-\s+(.+))?", line_content)
    ...
    def fetch_content(self, url: str) -> Document:
        """Fetch and parse an Anthropic documentation page.

        Anthropic URLs in llms.txt already include .md extension. This method
        ...
        return Document(
            url=url, content=content, metadata={"title": title, "source": "anthropic", "format": content_format}
        )
